...
- **Device Registry**: Device registration with automatic UUID-based QR code generation and live status tracking. *(implemented)*
- **Checkout & Reservation** *(planned)*: Scan-to-checkout, future-slot reservations, and auto-release for forgotten returns — all backed by a single active-checkout constraint per device.
- **Waitlist / Notify-me** *(planned)*: Queue for devices currently in use, with notification on return.
...
- **`User`**: Registered system users.
  ...
  - `role` *(planned)*: `EMPLOYEE` | `ADMIN`
  - `createdAt`, `updatedAt`

- **`Device`** *(renamed from `Product`)*: Hardware devices/assets being tracked.
  - `id` (`String`): Primary Key, mapping to `_id`.
  ...
  - `location` *(planned)*: e.g. "Accessories lab"
  - `createdAt`, `updatedAt`

- **`Checkout`** *(planned)*: A single table covering both immediate checkouts and future reservations.
...
- **`Waitlist`** *(planned)*: Notify-me queue for devices currently in use.
  - `id`, `deviceId` (FK), `userId` (FK)
...
#### Get Device by QR Code *(planned — the core scan endpoint)*
...
### 4. Checkout & Reservation Endpoints *(planned)*
...
- [x] Device registration + QR generation
- [ ] Rename `product` → `device` across routes/controllers/services (see structure above)
- [ ] `GET /device/qr/:qrCode` — the scan-resolution endpoint
- [ ] Checkout model + partial unique index (one active checkout per device)
- [ ] Reservation flow (future time slots)
- [ ] Auto-release job for forgotten returns
- [ ] Waitlist / notify-me-when-free
- [ ] Usage history per device
- [ ] Admin role + protected device management routes